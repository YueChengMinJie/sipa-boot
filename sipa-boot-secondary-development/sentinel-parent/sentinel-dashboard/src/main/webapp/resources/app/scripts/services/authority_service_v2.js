...
angular.module('sentinelDashboardApp').service('AuthorityRuleServiceV2', ['$http', function ($http) {
  this.queryMachineRules = function (app) {
    ...
    return $http({
      url: 'v2/authority/rules', params: param, method: 'GET'
    });
  };

  this.addNewRule = function (rule) {
    return $http({
      url: 'v2/authority/rule', data: rule, method: 'POST'
    });
  };

  this.saveRule = function (entity) {
    return $http({
      url: 'v2/authority/rule/' + entity.id, data: entity, method: 'PUT'
    });
  };

  this.deleteRule = function (entity) {
    return $http({
      url: 'v2/authority/rule/' + entity.id, method: 'DELETE'
    });
  };
