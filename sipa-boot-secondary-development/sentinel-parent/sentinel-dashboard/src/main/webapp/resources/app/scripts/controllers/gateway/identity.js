...
app.controller('GatewayIdentityCtl', ['$scope', '$stateParams', 'IdentityService', 'ngDialog', 'GatewayFlowServiceV2', 'GatewayApiServiceV2', 'DegradeService', 'MachineService', '$interval', '$location', '$timeout', function ($scope, $stateParams, IdentityService, ngDialog, GatewayFlowService, GatewayApiService, DegradeService, MachineService, $interval, $location, $timeout) {
  ...
  function getApiNames() {
    ...
    GatewayApiService.queryApis($scope.app, mac[0], mac[1]).success(function (data) {
  ...
  $scope.addNewGatewayFlowRule = function (resource) {
    ...
    gatewayFlowRuleDialogScope.intervalUnits = [{val: 0, desc: '秒'}, {val: 1, desc: '分'}, {
      val: 2, desc: '时'
    }, {val: 3, desc: '天'}];
    ...
    gatewayFlowRuleDialogScope.useParamItem = function () {
      gatewayFlowRuleDialogScope.currentRule.paramItem = {
        parseStrategy: 0, matchStrategy: 0
      };
    };
  ...
  $scope.addNewDegradeRule = function (resource) {
    ...
    degradeRuleDialogScope.degradeRuleDialog = {
      title: '新增降级规则', type: 'add', confirmBtnText: '新增', saveAndContinueBtnText: '新增并继续添加'
    };
