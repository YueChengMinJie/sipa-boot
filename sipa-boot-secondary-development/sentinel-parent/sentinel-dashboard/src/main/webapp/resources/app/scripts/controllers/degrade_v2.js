...
app.controller('DegradeCtlV2', ['$scope', '$stateParams', 'DegradeServiceV2', 'ngDialog', function ($scope, $stateParams, DegradeService, ngDialog) {
  //初始化
  ...
  function getMachineRules() {
    DegradeService.queryMachineRules($scope.app).success(function (data) {
  ...
  $scope.editRule = function (rule) {
    ...
    $scope.degradeRuleDialog = {
      title: '编辑熔断规则', type: 'edit', confirmBtnText: '保存'
    };
    degradeRuleDialog = ngDialog.open({
      template: '/app/views/dialog/degrade-rule-dialog.html', width: 680, overlay: true, scope: $scope
    });
  };

  $scope.addNewRule = function () {
    $scope.currentRule = {
      grade: 0, app: $scope.app, limitApp: 'default', minRequestAmount: 5, statIntervalMs: 1000,
    };
    $scope.degradeRuleDialog = {
      title: '新增熔断规则', type: 'add', confirmBtnText: '新增'
    };
    degradeRuleDialog = ngDialog.open({
      template: '/app/views/dialog/degrade-rule-dialog.html', width: 680, overlay: true, scope: $scope
    });
  ...
  $scope.deleteRule = function (rule) {
    ...
    $scope.confirmDialog = {
      ...
      attention: '资源名: ' + rule.resource + ', 熔断策略: ' + parseDegradeMode(rule.grade) + ', 阈值: ' + rule.count,
      confirmBtnText: '删除',
    };
