...
app.controller('GatewayFlowCtlV2', ['$scope', '$stateParams', 'GatewayFlowService', 'GatewayApiService', 'ngDialog', function ($scope, $stateParams, GatewayFlowService, GatewayApiService, ngDialog) {
  ...
  function getMachineRules() {
    GatewayFlowService.queryRules($scope.app).success(function (data) {
  ...
  $scope.editRule = function (rule) {
    ...
    $scope.gatewayFlowRuleDialog = {
      title: '编辑网关流控规则', type: 'edit', confirmBtnText: '保存'
    };
    gatewayFlowRuleDialog = ngDialog.open({
      template: '/app/views/dialog/gateway/flow-rule-dialog.html', width: 780, overlay: true, scope: $scope
    });
  };
  ...
  $scope.addNewRule = function () {
    ...
    $scope.gatewayFlowRuleDialog = {
      title: '新增网关流控规则', type: 'add', confirmBtnText: '新增'
    };
