...
app.controller('FlowControllerV2', ['$scope', '$stateParams', 'FlowServiceV2', 'ngDialog', function ($scope, $stateParams, FlowService, ngDialog) {
  ...
  function getMachineRules() {
    FlowService.queryMachineRules($scope.app).success(function (data) {
  ...
  $scope.editRule = function (rule) {
    ...
    flowRuleDialog = ngDialog.open({
      template: '/app/views/dialog/flow-rule-dialog.html', width: 680, overlay: true, scope: $scope
    });
  };
  ...
  $scope.addNewRule = function () {
    ...
    $scope.flowRuleDialog = {
      title: '新增流控规则', type: 'add', confirmBtnText: '新增', showAdvanceButton: true,
    };
    flowRuleDialog = ngDialog.open({
      template: '/app/views/dialog/flow-rule-dialog.html', width: 680, overlay: true, scope: $scope
    });
  ...
  $scope.deleteRule = function (rule) {
    ...
    $scope.confirmDialog = {
      ...
      attention: '资源名: ' + rule.resource + ', 流控应用: ' + rule.limitApp + ', 阈值类型: ' + (rule.grade == 0 ? '线程数' : 'QPS') + ', 阈值: ' + rule.count,
      confirmBtnText: '删除',
    };
