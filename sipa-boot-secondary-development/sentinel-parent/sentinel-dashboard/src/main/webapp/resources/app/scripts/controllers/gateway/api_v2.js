...
app.controller('GatewayApiCtlV2', ['$scope', '$stateParams', 'GatewayApiServiceV2', 'ngDialog', function ($scope, $stateParams, GatewayApiService, ngDialog) {
  ...
  function getApis() {
    GatewayApiService.queryApis($scope.app).success(function (data) {
  ...
  $scope.editApi = function (api) {
    ...
    $scope.gatewayApiDialog = {
      title: '编辑自定义 API', type: 'edit', confirmBtnText: '保存'
    };
    gatewayApiDialog = ngDialog.open({
      template: '/app/views/dialog/gateway/api-dialog.html', width: 900, overlay: true, scope: $scope
    });
  };

  $scope.addNewApi = function () {
    $scope.currentApi = {
      grade: 0, app: $scope.app, predicateItems: [{matchStrategy: 0, pattern: ''}]
    };
    $scope.gatewayApiDialog = {
      title: '新增自定义 API', type: 'add', confirmBtnText: '新增'
    };
    gatewayApiDialog = ngDialog.open({
      template: '/app/views/dialog/gateway/api-dialog.html', width: 900, overlay: true, scope: $scope
    });
