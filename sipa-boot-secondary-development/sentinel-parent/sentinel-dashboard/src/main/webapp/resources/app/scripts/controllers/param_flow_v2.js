 ...
 * @author Eric Zhao
 */
angular.module('sentinelDashboardApp').controller('ParamFlowControllerV2', ['$scope', '$stateParams', 'ParamFlowServiceV2', 'ngDialog', function ($scope, $stateParams, ParamFlowService, ngDialog) {
  const UNSUPPORTED_CODE = 4041;
  $scope.app = $stateParams.app;
  $scope.curExItem = {};

  $scope.paramItemClassTypeList = ['int', 'double', 'java.lang.String', 'long', 'float', 'char', 'byte'];
  ...
  $scope.addParamItem = () => {
    updateSingleParamItem($scope.currentRule.rule.paramFlowItemList, $scope.curExItem.object, $scope.curExItem.classType, $scope.curExItem.count);
  ...
  $scope.editRule = function (rule) {
    ...
    paramFlowRuleDialog = ngDialog.open({
      template: '/app/views/dialog/param-flow-rule-dialog.html', width: 680, overlay: true, scope: $scope
    });
    $scope.curExItem = {};
  };
  ...
  $scope.addNewRule = function () {
    ...
    $scope.paramFlowRuleDialog = {
      title: '新增热点规则', type: 'add', confirmBtnText: '新增', supportAdvanced: true, showAdvanceButton: true,
    };
    paramFlowRuleDialog = ngDialog.open({
      template: '/app/views/dialog/param-flow-rule-dialog.html', width: 680, overlay: true, scope: $scope
    });
  ...
  $scope.deleteRule = function (ruleEntity) {
    ...
    $scope.confirmDialog = {
      ...
      attention: '资源名: ' + ruleEntity.rule.resource + ', 热点参数索引: ' + ruleEntity.rule.paramIdx + ', 限流模式: ' + (ruleEntity.rule.grade === 1 ? 'QPS' : '未知') + ', 限流阈值: ' + ruleEntity.rule.count,
      confirmBtnText: '删除',
    };
