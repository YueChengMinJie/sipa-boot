 ...
 */
angular.module('sentinelDashboardApp').controller('AuthorityRuleControllerV2', ['$scope', '$stateParams', 'AuthorityRuleServiceV2', 'ngDialog', function ($scope, $stateParams, AuthorityRuleService, ngDialog) {
  ...
  $scope.editRule = function (rule) {
    ...
    $scope.authorityRuleDialog = {
      title: '编辑授权规则', type: 'edit', confirmBtnText: '保存',
    };
    authorityRuleDialog = ngDialog.open({
      template: '/app/views/dialog/authority-rule-dialog.html', width: 680, overlay: true, scope: $scope
    });
  };
  ...
  $scope.addNewRule = function () {
    $scope.currentRule = {
      app: $scope.app, rule: {
        strategy: 0, limitApp: 'default'
      }
    };
    $scope.authorityRuleDialog = {
      title: '新增授权规则', type: 'add', confirmBtnText: '新增', showAdvanceButton: true,
    };
    authorityRuleDialog = ngDialog.open({
      template: '/app/views/dialog/authority-rule-dialog.html', width: 680, overlay: true, scope: $scope
    });
  ...
  $scope.deleteRule = function (ruleEntity) {
    ...
    $scope.confirmDialog = {
      ...
      attention: '资源名: ' + ruleEntity.rule.resource + ', 流控应用: ' + ruleEntity.rule.limitApp + ', 类型: ' + (ruleEntity.rule.strategy === 0 ? '白名单' : '黑名单'),
      confirmBtnText: '删除',
    };
