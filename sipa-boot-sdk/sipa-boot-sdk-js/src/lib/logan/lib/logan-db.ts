import { CustomDB, idbIsSupported } from "../../idb-managed"
import {
  dateFormat2Day,
  dayFormat2Date,
  getStartOfDay,
...
export const LOG_DETAIL_TABLE_NAME = "logan_detail_table"
export const LOG_DAY_TABLE_NAME = "log_day_table"
const LOG_DETAIL_REPORT_NAME_INDEX = "logReportName"
const LOG_DETAIL_CREATE_TIME_INDEX = "logCreateTime"
export const LOG_DAY_TABLE_PRIMARY_KEY = "logDay"
export type FormattedLogReportName = string

const DEFAULT_LOG_DURATION = 30 * 24 * 3600 * 1000 // logan-web keeps 7 days logs locally
const DEFAULT_SINGLE_DAY_MAX_SIZE = 10 * M_BYTE // 7M storage limit for one day
const DEFAULT_SINGLE_PAGE_MAX_SIZE = M_BYTE // 1M storage limit for one page
...
interface LoganLogItem {
  [LOG_DETAIL_REPORT_NAME_INDEX]: string
  [LOG_DETAIL_CREATE_TIME_INDEX]: TimeStamp
  logSize: number
  logString: string
}
...
export default class LoganDB {
    ...
    this.DB = new CustomDB({
      ...
      tables: {
        [LOG_DETAIL_TABLE_NAME]: {
          indexList: [
            {
              indexName: LOG_DETAIL_REPORT_NAME_INDEX,
              unique: false,
            },
            {
              indexName: LOG_DETAIL_CREATE_TIME_INDEX,
              unique: false,
            },
          ],
        },
  ...
  async addLog(logString: string): Promise<void> {
    ...
    const todayInfo: LoganLogDayItem = (await this.getLogDayInfo(today)) || {
    ...
    const logItem: LoganLogItem = {
      [LOG_DETAIL_REPORT_NAME_INDEX]: this.logReportNameFormatter(
        today,
        needNewPage ? currentPageIndex + 1 : currentPageIndex
      ),
      [LOG_DETAIL_CREATE_TIME_INDEX]: +now,
      logSize,
      logString,
    }
    ...
    // The expiry time is the start of the day after 7 days.
  ...
  async incrementalDelete(
    ...
    if (dayInfo && dayInfo.reportPagesInfo) {
      ...
      const pageSizesArrayWithNewPage =
        (function addNewPageIfLastPageIsReported(): number[] {
          // Add a new page with 0 page size if the last page is reported.
          if (
            reportedPageIndexes.indexOf(currentPageSizesArr.length - 1) >= 0
          ) {
