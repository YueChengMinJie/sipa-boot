  ...
  if (logItems.length > 0) {
    ...
    return await Ajax(
      ...
      if (typeof customXHROpts.responseDealer === "function") {
        ...
        if (result.resultMsg === ResultMsg.REPORT_LOG_SUCCESS) {
          return pageIndex
        } else {
        ...
        if (response?.errorCode === "000000") {
          ...
          throw new Error(`Server error, errorCode: ${response?.errorCode}`)
        }
      }
    })
  } else {
      ...
      const logReportMap: {
        ...
        return {
          [logDayInfo[LOG_DAY_TABLE_PRIMARY_KEY]]: logDayInfo.reportPagesInfo
            ? logDayInfo.reportPagesInfo.pageSizes.map((_, pageIndex) => {
      ...
      for (
        ...
        if (logReportMap[logDay] && logReportMap[logDay].length > 0) {
          try {
            ...
            reportResult[logDay] = { msg: ResultMsg.REPORT_LOG_SUCCESS }
            try {
