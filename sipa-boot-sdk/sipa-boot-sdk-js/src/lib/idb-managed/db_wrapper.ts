...
function upgradeDBWithTableList(
  ...
  try {
    tableList.forEach((tableConfig) => {
      ...
      if (
        upgradeDB.objectStoreNames.contains(tableConfig.tableName as string)
      ) {
        ...
        ;(tableConfig.indexList || []).forEach((theIndex: IndexOfTable) => {
        ...
        const tableToCreate = upgradeDB.createObjectStore(
          tableConfig.tableName as string,
          {
            ...
            ...(tablePrimaryKey === "id" ? { autoIncrement: true } : {}),
          }
        )
...
export async function addItems(dbInfo: DB, items: ItemConfig[]) {
  ...
  await atomicTrans(addItemsTrans, db, async () => {
    await Promise.all(
      items.map((item) =>
        addItemsTrans.objectStore(item.tableName).put(itemWrapper(item))
      )
    )
  })
}
      ...
      if (!db.objectStoreNames.contains(tableName)) {
        ...
        try {
          ...
          if (!indexRange) {
            ...
            const wrappedItems = await table.getAll()
            items = (wrappedItems || [])
              .map(itemUnwrapper)
              .filter((item: any) => {
                return item !== null
            ...
            await new Promise(function (resolve) {
              table
                ...
                .iterateCursor(indexRange2DBKey(indexRange), (cursor: any) => {
                  ...
                  const item = itemUnwrapper(cursor.value)
